...
test('E-35: 로스터리를 북마크하면 즐겨찾기 페이지에 표시된다', async ({ page }) => {
  await loginComplete(page, `e2e-bookmark-add-${Date.now()}@roco.dev`)
...
test('E-36: 즐겨찾기 페이지에서 북마크를 해제할 수 있다', async ({ page }) => {
  await loginComplete(page, `e2e-bookmark-remove-${Date.now()}@roco.dev`)
  await page.goto('/roasteries')
  await page.waitForLoadState('networkidle')
...
test('E-37: 즐겨찾기가 없으면 빈 상태 안내 메시지가 표시된다', async ({ page }) => {
  await loginComplete(page, `e2e-bookmark-empty-${Date.now()}@roco.dev`)
  await page.goto('/bookmarks')
  await page.waitForLoadState('networkidle')
...
test('E-38: 로스터리 상세에서 북마크 버튼이 동작한다', async ({ page }) => {
  await loginComplete(page, `e2e-bookmark-detail-${Date.now()}@roco.dev`)
  await page.goto('/roasteries')
