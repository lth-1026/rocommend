...
test('E-50: /account 페이지에 프로필 사진 변경 UI가 표시된다', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /사진 변경|프로필 사진/ }).first()).toBeVisible()
})
...
test('E-51: 사진 변경 버튼 클릭 시 파일 피커가 열린다', async ({ page }) => {
  ...
  const [fileChooser] = await Promise.all([
    ...
    page.getByRole('button', { name: /사진 변경|프로필 사진/ }).first().click(),
  ])
...
test('E-52: 유효한 이미지 파일 선택 시 업로드가 성공하고 프로필 사진이 업데이트된다', async ({ page }) => {
  ...
  // 업로드 완료: 버튼 비활성화 해제 + 오류 없음
  await expect(page.getByRole('button', { name: '프로필 사진 변경' })).not.toBeDisabled({ timeout: 10000 })
})
...
test('E-54: 파일 선택 후 업로드 중 로딩 상태가 표시된다', async ({ page }) => {
  ...
  // 업로드 중: 버튼 비활성화 상태
  await expect(page.getByRole('button', { name: '프로필 사진 변경' })).toBeDisabled({ timeout: 3000 })
})
