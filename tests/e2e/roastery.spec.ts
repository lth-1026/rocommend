...
test('E-18: 가격대 필터 선택 시 URL에 price 파라미터가 반영된다', async ({ page }) => {
  ...
  await page.getByRole('checkbox', { name: '2만원 미만' }).check()
...
test('E-20: 디카페인 필터 선택 시 URL에 decaf=1이 반영된다', async ({ page }) => {
  ...
  await page.getByRole('checkbox', { name: '디카페인 가능' }).check()
...
test('E-21: 지역 필터 선택 시 URL에 region 파라미터가 반영된다', async ({ page }) => {
  ...
  await page.getByRole('checkbox', { name: '부산' }).check()
  await expect(page).toHaveURL(/region=부산/)
})
