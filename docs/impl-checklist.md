...
| 피처 | 브랜치 | 상태 | 테스트 게이트 |
|------|--------|------|--------------|
| 사전 작업 (스키마·인프라) | `chore/pre-impl` | 대기 | U-01~06 |
| F-001 소셜 로그인 | `feat/auth-login` | 대기 | U-42~49, C-25~26, E-01~05 |
| F-002 온보딩 설문 | `feat/onboarding` | 대기 | U-26~30, C-20~24, I-01~04, E-06~12 |
| F-003 로스터리 목록/상세 | `feat/roastery-list-detail` | 대기 | C-01~08, E-13~17 |
| F-004 검색/필터 | `feat/roastery-filter` | 대기 | I-18~23, E-18~24 |
| F-005 평가 시스템 | `feat/rating-system` | 대기 | U-31~37, C-09~15, I-05~09, E-25~29 |
| F-007 즐겨찾기 | `feat/bookmark` | 대기 | U-38~41, C-16~19, I-10~12, E-35~39 |
| F-006 CF 추천 홈 피드 | `feat/home-feed-cf` | 대기 | U-07~25, I-13~17, A-01~05, E-30~34 |
| F-008 사용자 프로필 | `feat/user-profile` | 대기 | E-40~44 |
| 이벤트 로깅 | `feat/event-logging` | 대기 | (I-17, E-33 재확인) |

> 상태 값: `대기` → `진행중` → `구현완료` → ✅ `테스트통과`
...
**GIN 인덱스 수동 추가** (배열 컬럼 `hasSome` 필터 성능용 — B-Tree로는 배열 내부 검색 불가) (Prisma 미지원, 마이그레이션 SQL에 직접 추가)
...
통과 기준: **U-01 ~ U-06** (formatRegions 6개 케이스 전부 green)
...
### 보안
- [ ] 모든 Server Action: `auth()`로 서버에서 `userId` 직접 조회 — 클라이언트가 `userId` 전송 시 무시 (조작 방지)
- [ ] 모든 Server Action: Zod로 입력값 서버 사이드 검증
...
# 컴포넌트: Navigation breakpoint
pnpm test src/components/layout/Navigation.test.tsx
# E2E: 인증 플로우
pnpm test:e2e tests/e2e/auth.spec.ts
```

통과 기준: **U-42 ~ U-49** (proxy 8개) + **C-25, C-26** (Navigation 2개) + **E-01 ~ E-05** (auth E2E 5개)
...
### 컴포넌트
- [ ] `src/components/onboarding/OnboardingWizard.tsx` — `'use client'`, `useState`로 스텝·답변 관리
...
```bash
# 단위: submitOnboarding Server Action
pnpm test src/actions/onboarding.test.ts
# 컴포넌트: OnboardingForm 스텝 흐름
pnpm test src/components/onboarding/OnboardingWizard.test.tsx
# 통합: 온보딩 DB 저장 + Rating 생성
pnpm test src/actions/onboarding.integration.test.ts
# E2E: 온보딩 전체 플로우
pnpm test:e2e tests/e2e/onboarding.spec.ts
```

통과 기준: **U-26 ~ U-30** (submitOnboarding 5개) + **C-20 ~ C-24** (OnboardingForm 5개) + **I-01 ~ I-04** (통합 4개) + **E-06 ~ E-12** (E2E 7개)
...
```bash
# 컴포넌트: RoasteryCard (지역 표시 포함)
pnpm test src/components/roastery/RoasteryCard.test.tsx
# E2E: 목록 → 상세 네비게이션
pnpm test:e2e tests/e2e/roastery.spec.ts
```

통과 기준: **C-01 ~ C-08** (RoasteryCard 8개) + **E-13 ~ E-17** (roastery E2E 5개)
...
```bash
# 통합: 필터 조합 (GIN index 포함)
pnpm test src/actions/roastery.integration.test.ts
# E2E: 필터 UI + URL 상태 유지
pnpm test:e2e tests/e2e/filter.spec.ts
```

통과 기준: **I-18 ~ I-23** (통합 6개) + **E-18 ~ E-24** (filter E2E 7개)
...
```bash
# 단위: upsertRating / deleteRating Server Action
pnpm test src/actions/rating.test.ts
# 컴포넌트: RatingForm (별점 선택 + 제출)
pnpm test src/components/rating/RatingForm.test.tsx
# 통합: 평가 DB 저장 + 평가수 3개 CF 트리거
pnpm test src/actions/rating.integration.test.ts
# E2E: 평가 제출 / 수정 / 삭제
pnpm test:e2e tests/e2e/rating.spec.ts
```

통과 기준: **U-31 ~ U-37** (rating 단위 7개) + **C-09 ~ C-15** (RatingForm 7개) + **I-05 ~ I-09** (통합 5개) + **E-25 ~ E-29** (rating E2E 5개)
...
### 컴포넌트
- [ ] `src/components/bookmark/BookmarkButton.tsx` — 하트 아이콘 토글 (`useOptimistic` 낙관적 업데이트 — 서버 응답 전 즉시 반영, 실패 시 롤백)
...
```bash
# 단위: toggleBookmark Server Action
pnpm test src/actions/bookmark.test.ts
# 컴포넌트: BookmarkButton (optimistic 포함)
pnpm test src/components/bookmark/BookmarkButton.test.tsx
# 통합: 북마크 추가/제거 + 목록 조회
pnpm test src/actions/bookmark.integration.test.ts
# E2E: 북마크 토글 → 즐겨찾기 페이지
pnpm test:e2e tests/e2e/bookmark.spec.ts
```

통과 기준: **U-38 ~ U-41** (bookmark 단위 4개) + **C-16 ~ C-19** (BookmarkButton 4개) + **I-10 ~ I-12** (통합 3개) + **E-35 ~ E-39** (bookmark E2E 5개)
...
```bash
# 단위: CF 코사인 유사도 + 추천 로직
pnpm test src/lib/recommender/item-cf.test.ts
# 단위: 인기 폴백
pnpm test src/lib/recommender/popular.test.ts
# 단위: getRecommendations (조건 분기)
pnpm test src/lib/recommender/index.test.ts
# 통합: CF 실행 + Recommendation 저장
pnpm test src/lib/recommender/recommender.integration.test.ts
# 추천 정확도 offline evaluation
pnpm test src/lib/recommender/accuracy.test.ts
# E2E: 홈 피드 (폴백 → CF 전환)
pnpm test:e2e tests/e2e/home-feed.spec.ts
```

통과 기준: **U-07 ~ U-25** (CF 단위 19개) + **I-13 ~ I-17** (통합 5개) + **A-01 ~ A-05** (정확도 5개) + **E-30 ~ E-34** (home-feed E2E 5개)

---

## F-008 사용자 프로필 `feat/user-profile`

소셜 로그인 전용이므로 편집 가능한 정보 없음. 내 활동 요약 + 로그아웃 중심.

### 페이지
- [ ] `src/app/(main)/profile/page.tsx` — Server Component

### 컴포넌트
- [ ] `src/components/profile/ProfileCard.tsx` — 아바타(OAuth 이미지), 이름, 이메일
- [ ] `src/components/profile/ActivitySummary.tsx` — 내 평가 수, 즐겨찾기 수 요약
- [ ] `src/components/profile/LogoutButton.tsx` — `'use client'`, `signOut()` 호출

### 데이터 레이어
- [ ] `src/lib/queries/profile.ts` — `getProfileSummary(userId)`:
  - 평가 수 (`_count`)
  - 즐겨찾기 수 (`_count`)
  - 단일 쿼리로 집계

### Header 연결
- [ ] `Header.tsx` — 사용자 아바타 클릭 → 드롭다운 (프로필 링크 + 로그아웃)

### ✅ F-008 테스트 게이트

```bash
# E2E: 프로필 페이지 + 로그아웃
pnpm test:e2e tests/e2e/profile.spec.ts
```

통과 기준: **E-40 ~ E-44** (profile E2E 5개)
